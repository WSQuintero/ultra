...
function Profile() {
  ...
  const handleFormSubmit = useCallback(
    async (event) => {
      ...
      if (!user.firstname || !user.lastname || !user.email || !user.slug_invitation || !user.cellphone) {
        setAlert({
          show: true,
  ...
  const handleFormSubmitWallet = useCallback(
    async (event) => {
      ...
      if (!expresionOneRegularTRC20.test(user.wallet_address_pay_commission)&&!expresionTwoRegularTRC20.test(user.wallet_address_pay_commission)) {
        setAlert({
          show: true,
  ...
  useEffect(() => {
    ...
    if (session) {
      setUser({
        ...
        email: session.email || "",
        slug_invitation: `${import.meta.env.VITE_APP_URL}/signup/${session.slug_invitation}` || "",
        cellphone: session.cellphone || "",
        contract_balance_own: Number(session.contract_balance_own) || 0,
        wallet_address_pay_commission: session.wallet_address_pay_commission || "",
        contract_balance_leveraged: Number(session.contract_balance_leveraged) || 0,
      });
    }
      ...
      <Container maxWidth="xl">
        <Box borderRadius={4} sx={{ backgroundColor: "white" }}>
          <Box position="relative" paddingX={2} paddingY={4}>
            ...
            >
              <img src={background} alt="cover photo" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </Box>
            <Grid position="absolute" bottom={0} left={48} display="flex" alignItems="flex-start" gap={2}>
              ...
              >
                ...
                <Avatar src={`${import.meta.env.VITE_IMAGES_URL}/${user.avatar}`} alt={user.firstname} sx={{ width: 92, height: 92 }} />
          ...
          {session.exist_2fa_auth==1 && (<Grid
              ...
              noValidate
            >
              <FormRow title="Google Authenticator - ACTIVO" description="Tus transacciones están seguras!"></FormRow>
            </Grid>)}
            
            {session.exist_2fa_auth==0 && (<Grid
              component="form"
              display="flex"
              flexDirection="column"
              padding={2}
              ...
              noValidate
            >
              <FormRow title="Google Authenticator - INACTIVO" description="Asegura todas tus transacciones"></FormRow>
              ...
              >
                <LoadingButton onClick={()=>{ setModalConfirm2Fa(true); }} variant="contained" fullWidth>
          ...
          <Grid
            ...
            noValidate
          >
            <FormRow title="Dirección Billetera TRC-20" description="Billetera para pago de retiros">
              <TextField
          ...
          {modalConfirmAuth&&(<ModalAuth
            ...
            handleAuth={async (pass) => {
              ...
              if (status) {
                ...
                setAlert({
                  show: true,
                  message: "Error al actualizar billetera, inténtelo de nuevo más tarde.",
                  status: "error",
                });
              }
            }}
          ...
          <Grid
            ...
            noValidate
          >
            <FormRow title="Status Binario" description="Membresía IB Activa + 2 Directos con Standar o Premium Activa">
              <TextField
            ...
            <FormRow title="Enlace de referido" description="Actualiza tu enlace de referido">
              <TextField
                name="slug_invitation"
            ...
            <FormRow title="Correo" description="Actualiza tu correco electrónico">
              <TextField name="email" value={user.email} onChange={handleInputChange} fullWidth disabled />
            </FormRow>
            <FormRow title="Nombre" description="Actualiza tu nombre">
              <TextField name="firstname" value={user.firstname} onChange={handleInputChange} fullWidth />
            </FormRow>
            <Divider flexItem />
            <FormRow title="Apellido" description="Actualiza tu apellido">
              <TextField name="lastname" value={user.lastname} onChange={handleInputChange} fullWidth />
            </FormRow>
            <Divider flexItem />
            <Divider flexItem />
            <FormRow title="Teléfono" description="Actualiza tu número de teléfono">
              <TextField name="cellphone" value={user.cellphone} onChange={handleInputChange} fullWidth />
            </FormRow>
            <FormRow title="Status Binario" description="Membresía IB Activa + 2 Directos con Standar o Premium Activa">
              <TextField
                name="slug_invitation"

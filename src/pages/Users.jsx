...
function Users() {
  ...
  const [modalGiveCommissionsOpen, setModalGiveCommissionsOpen] = useState(false);
  const [modalGiveCommissionsOpenPass, setModalGiveCommissionsOpenPass] = useState('');
  ...
  const headCells = [
    ...
    },
    {
      ...
      width: "14.2%",
      disablePadding: false,
      format: (value, row) => (value === 1 ? "Verificado" : (<Button variant="contained" onClick={() =>{ setModalConfirmAuth(`6-${row.id}`); }}>Sin verificar - Enviar Email</Button>)),
    ...
    }, 
    {
      ...
      width: "14.2%",
      disablePadding: false,
      format: (value, row) => (Number(value) > 0 ? (<b>ACTIVA</b>) : (<Button variant="contained" onClick={() =>{ setModalConfirmAuth(`4-${row.id}`); }}>Activar</Button>)),
    },
    {
      id: "standar",
      label: "M. Estándar",
      width: "14.2%",
      disablePadding: false,
      format: (value, row) => (Number(value) > 0 ? (<b>ACTIVA</b>) : (<Button variant="contained" onClick={() =>{ setModalConfirmAuth(`1-${row.id}`); }}>Activar</Button>)),
    },
    {
      id: "premium",
      label: "M. Premium",
      width: "14.2%",
      disablePadding: false,
      format: (value, row) => (Number(value) > 0 ? (<b>ACTIVA</b>) : (<Button variant="contained" onClick={() =>{ setModalConfirmAuth(`2-${row.id}`); }}>Activar</Button>)),
    },
    {
      id: "id",
      label: "Operaciones",
      ...
      format: (value, row) => {
          ...
          >
            ...
            <MenuItem value={(row.gived ? 8 : 3)}>{row.gived ? 'Otorgar Depósito' : 'Retirar Depósito'}</MenuItem>
            <MenuItem value={9}>Cambiar Contraseña</MenuItem>
            <MenuItem value={10}>Mover árbol - Actualmente bajo: {row.ref}</MenuItem>
            <MenuItem value={11}>Cambiar directo - Actualmente: {row.dir}</MenuItem>
          </Select>);
        },
    }
  ];
  ...
  return (
    <PageWrapper sx={{ padding: 2 }}>
      {users && <EnhancedTable title="Usuarios" headCells={headCells} rows={users} csv={true} onSearch={(val)=>{ 
          let toFilterUsers = usersOriginal;
          let newUsers = [];
          toFilterUsers.filter((user)=>{
            if(user.id==val||user.fullName.toLowerCase().indexOf(val)>=0||user.email.toLowerCase().indexOf(val)>=0) newUsers.push(user);
          });
      ...
      {modalConfirmAuth&&(<ModalAuth
        ...
        handleAuth={async (password) => {
          ...
          if(idProduct==10||idProduct==11){
            ...
            const { status, data } = await $Subscription.removeDeposit({idUser, password});
            ...
            if (status) {
              ...
              setAlert({
                show: true,
                message: "Error al activar la membresía, inténtelo de nuevo más tarde.",
                status: "error",
              });
            }
      ...
      <ModalGiveCommissions
        ...
        handleOk={async (originCommission, amount) => {
          ...
          if (status) {
            ...
            setAlert({
              show: true,
              message: "Error al asignar la comisión, inténtelo de nuevo más tarde.",
              status: "error",
            });
          }
      ...
      <ModalGiveDeposit
        ...
        handleOk={async (amount) => {
          ...
          if (status) {
            ...
            setAlert({
              show: true,
              message: "Error al asignar el depósito, inténtelo de nuevo más tarde.",
              status: "error",
            });
          }

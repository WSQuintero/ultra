...
function Reports() {
  ...
  const totalProfits = useMemo(() => profits?.reduce((a, c) => a + parseFloat(c.amount_profit), 0) || 0, [profits]);
  ...
  const fetchAdminProfits = async () => {
    const { status, data } = await $Subscription.getProfits({ isAdmin: true, pending: true });
  ...
  return (
    <PageWrapper sx={{ display: "flex", flexDirection: "column", gap: 2, padding: 2 }}>
      <Grid display="flex" flexDirection="column" gap={2}>
        {profits && (<>
          ...
          <EnhancedTable
            ...
            rows={profits}
            footer={
              <Grid display={isMobile ? "block" : "flex"} textAlign={'center'} alignItems={'center'} gap={1}>
        ...
        {session?.rol === 1 && adminProfits && (
          <EnhancedTable
            ...
            rows={adminProfits}
            footer={
              <Grid display="flex"  textAlign={'center'} alignItems={'center'} gap={1}>
                <Typography fontWeight={600} color="primary">
                  Total:
                </Typography>

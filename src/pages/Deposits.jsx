...
const headCells = [
  ...
  },
  {
    ...
    format: (value, row) => {
      ...
      let acelerateDeposit = Number(row.profit_direct_payed) + Number(row.profit_leverage_payed);
      let percentCompleted = acelerateDeposit > 0 ? Number((acelerateDeposit / originalDeposit) * 100) : 0;
    ...
    (async () => {
      ...
      if (status) {
        ...
        data.user.userSubscription.map((item) => {
          ...
          if ([1, 2].indexOf(item.product) >= 0 && Number(item.status_pay) == 1 && errorDeposit) {
            errorDeposit = false;
          }
        });
        ...
        >
          ...
          >
            <TextField
              ...
              fullWidth
            />
            <Button variant="contained" color="warning" onClick={() => buyPlanWithUSDT()} fullWidth>
              Nuevo depósito con USDT (TRC20)
            </Button>
          </Grid>
        </Box>

        {reports && <EnhancedTable title="Depósitos" headCells={headCells} rows={reports} />}
      </Grid>

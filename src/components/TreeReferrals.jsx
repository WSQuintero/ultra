...
import { ArrowBack as LeftIcon, ArrowForward as RightIcon, Person as PersonIcon } from "@mui/icons-material";
...
function TreeReferrals() {
  ...
  const [selectedPendingReferral, setSelectedPendingReferral] = useState("select");
  ...
  const foreignObjectProps = { width: nodeSize.x, height: nodeSize.y, x: -nodeSize.x / 2, y: -nodeSize.y / 2.5 };
  ...
  const buildTree = useCallback((referral, path, emailSearchP) => {
    ...
    const left = referral.binary.left ? buildTree(referral.binary.left, [...path, "left"]) : { name: "empty" };
    const right = referral.binary.right ? buildTree(referral.binary.right, [...path, "right"]) : { name: "empty" };
  ...
  const updateTree = async ({ name, children, attributes }) => {
    if (name !== "empty") {
      const newTree = { ...tree };
      const parentNode = attributes.path.reduce((a, c) => a.children[c === "left" ? 0 : 1], newTree);
  ...
  const handleAssignReferral = async (side) => {
    ...
    const { status, data } = await $Referral.assing({ idUser: selectedPendingReferral, side });
  ...
  return (
    ...
    >
      <Grid position="absolute" top={16} left={16} display="flex" flexDirection="column" alignItems="stretch" gap={2}>
        <Grid
          display="flex"
          alignItems="center"
        ...
        >
          <Grid display="flex" alignItems="center" gap={1}>
            <Box width={16} height={16} borderRadius={50} sx={(t) => ({ backgroundColor: t.palette.primary.main })} />
            <Typography>Directos</Typography>
          </Grid>
          <Grid display="flex" alignItems="center" gap={1}>
            <Box width={16} height={16} borderRadius={50} sx={(t) => ({ backgroundColor: t.palette.secondary.main })} />
        ...
        >
          <Typography color={session.activeBinary ? "#ff780a" : "textPrimary"}>
            <b>{session.activeBinary ? "BINARIO ACTIVO" : "BINARIO INACTIVO"}</b>
          </Typography>
          <Typography>
            Línea A (Izquierda): <b>Cuentas {treeData ? treeData.usersLeft : 0}</b> <br />
            <b>Disponible {amountLeftline ? amountLeftline.toLocaleString() : 0} USDT</b>{" "}
            <b>Total {treeData ? treeData.amountLeftOriginal.toLocaleString() : 0} USDT</b> <br />
          </Typography>
          <Typography>
            Línea B (Derecha): <b>Cuentas {treeData ? treeData.usersRight : 0}</b> <br />
            <b>Disponible {amountRightline ? amountRightline.toLocaleString() : 0} USDT</b>{" "}
            <b>Total {treeData ? treeData.amountRightOriginal.toLocaleString() : 0} USDT</b> <br />
          </Typography>
          <hr/>
          <br />
          <Typography>
            ...
            <Tree
              ...
              renderCustomNodeElement={({ nodeDatum, hierarchyPointNode }) => {
                if (nodeDatum.name !== "empty") {
                  hierarchyPointNode.data.__rd3t.collapsed = nodeDatum.attributes.collapsed;
                  return (
                    <g>
                      <foreignObject
                        {...foreignObjectProps}
                      ...
                      >
                        ...
                        >
                          ...
                          {nodeDatum.attributes.avatar?(<Avatar
                            sx={{
                              width: 48,
                              height: 48,
                          ...
                          >
                            <PersonIcon
                              color={nodeDatum.attributes.isDirect ? "primary" : "secondary"}
                              sx={{ strokeWidth: 0 }}
                            />
                          </Avatar>)}
                          <Grid
                          ...
                          >
                            <Typography fontWeight={700} fontSize={18} lineHeight={1} color="black" whiteSpace="nowrap">
                              {nodeDatum.attributes.name}
                            </Typography>
                            <Typography fontWeight={300} fontSize={16} lineHeight={1} color="black">
                          ...
                          {nodeDatum.attributes.itsPayed && (
                            ...
                            >
                              <SvgIcon sx={{ width: 16, height: 16 }}>{CityIcon}</SvgIcon>
                            </Box>
                          )}
                        </Box>
                      </foreignObject>
